...
function websiteTravelIntent(text){
  ...
  let m=t.match(/\bfrom\s+([A-Za-z][A-Za-z .'-]{1,45}?)\s+to\s+([A-Za-z][A-Za-z .'-]{1,45}?)(?=\s+(?:on|for|depart|departure|return|check|search|show|find|all|available|flight|flights|with|and)\b|[,.]|$)/i);
  if(!m)m=t.match(/\b([A-Za-z][A-Za-z .'-]{1,35}?)\s+to\s+([A-Za-z][A-Za-z .'-]{1,35}?)(?=\s+(?:on|for|depart|departure|return|check|search|show|find|available|flight|flights)\b|[,.]|$)/i);
  if(m){origin=m[1].replace(/^(?:go|fly|flight|flights|select|search|available|from)\s+/i,'').trim();destination=m[2].trim();}
