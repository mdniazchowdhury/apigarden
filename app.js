...
let userStore = {};
const websiteVoiceState={free:{listening:false,manualStop:false,transcript:'',draft:null,lastError:''},pro:{listening:false,manualStop:false,transcript:'',draft:null,lastError:''}};
let websiteVoiceRecognition=null;
let websiteVoiceFinal='';
...
async function loginFree(){
  ...
  go('free-app');
  renderWizard('free'); renderMyApis('free'); renderMarket('free'); renderWebsiteVoice('free'); updateCreditChip(); checkAIBackend('free');
}
async function loginPro(){
  ...
  go('pro-app');
  renderWizard('pro'); renderMyApis('pro'); renderMarket('pro'); renderWebsiteVoice('pro'); checkAIBackend('pro');
}
function loginAdmin(){
  loadAppData();
...
function switchTab(role, btn, name){
  ...
  if(name==='voice') renderWebsiteVoice(role);
}
...
function normalizeWebsiteVoice(v){return String(v||'').replace(/\s+/g,' ').trim();}
function websiteVoiceBestSite(text){
  const t=String(text||'').toLowerCase();
  const rules=[[/\b(shoe|shoes|sneaker|sneakers|sandal|sandals|loafer|loafers|boot|boots|footwear)\b/,'bata'],[/\b(book|books|novel|textbook|stationery|author)\b/,'rokomari'],[/\b(phone|mobile|smartphone|laptop|tablet|headphone|earphone|camera|smartwatch|gadget)\b/,'pickaboo'],[/\b(fridge|refrigerator|freezer|television|tv|ac|air conditioner|washing machine|water heater|home appliance)\b/,'walton'],[/\b(flight|flights|airfare|airline|airport|travel)\b/,'trip']];
  const m=rules.find(([rx])=>rx.test(t)); const key=m?m[1]:'daraz'; return {...WEBSITE_VOICE_SITES[key],key,domain:new URL(WEBSITE_VOICE_SITES[key].base).hostname,autoSelected:true};
}
function websiteVoiceDetectSite(text){
  const lower=text.toLowerCase();
  for(const [key,site] of Object.entries(WEBSITE_VOICE_SITES)) if(new RegExp(`\\b${key}\\b`,'i').test(lower)) return {...site,key,domain:new URL(site.base).hostname};
  const u=text.match(/(?:https?:\/\/)?(?:www\.)?([a-z0-9-]+(?:\.[a-z0-9-]+)+)(?:\/[^\s]*)?/i);
  if(u){const domain=u[1].toLowerCase(),base=`https://${domain}`;return {key:domain.split('.')[0],label:domain,base,domain,search:`${base}/search?q={query}`};}
  const n=text.match(/(?:go to|open|from|on|visit)\s+(?:the\s+)?([a-z0-9&' -]{2,40}?)\s+(?:website|site)\b/i)||text.match(/\b([a-z0-9&'-]{2,30})\s+(?:website|site)\b/i);
  if(n){const key=n[1].toLowerCase().replace(/\b(the|official)\b/g,'').trim().replace(/[^a-z0-9]/g,''); if(key){const base=`https://www.${key}.com`;return {key,label:n[1].trim(),base,domain:`www.${key}.com`,search:`${base}/search?q={query}`};}}
  return null;
}
function websiteVoicePrices(text){const t=text.toLowerCase().replace(/,/g,'');let minPrice=null,maxPrice=null,m=t.match(/(?:range|price(?:\s+range)?|from)?\s*(\d+(?:\.\d+)?)\s*(?:-|–|—|to|and)\s*(\d+(?:\.\d+)?)\s*(?:tk|taka|bdt)?\b/i);if(m){minPrice=Number(m[1]);maxPrice=Number(m[2]);}if(maxPrice==null){m=t.match(/(?:under|below|less than|up to|maximum|max)\s*(?:tk|taka|bdt)?\s*(\d+(?:\.\d+)?)/i);if(m){minPrice=0;maxPrice=Number(m[1]);}}if(minPrice==null){m=t.match(/(?:above|over|more than|minimum|min)\s*(?:tk|taka|bdt)?\s*(\d+(?:\.\d+)?)/i);if(m)minPrice=Number(m[1]);}return {minPrice,maxPrice};}
function websiteVoiceQuery(text,site){let q=text.replace(/https?:\/\/\S+/ig,' ').replace(/\b(?:go to|open|visit|search|find|show me|create|make|build|generate|an|a|the|api|website|site|please|for me|any|best|choose)\b/ig,' ');if(site){q=q.replace(new RegExp(site.key.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'ig'),' ');q=q.replace(new RegExp(String(site.label||'').replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'ig'),' ');}q=q.replace(/\b(?:price|range|priced|cost|between|from|under|below|above|over|less than|more than|up to|maximum|max|minimum|min)\b/ig,' ').replace(/\b\d[\d,]*(?:\.\d+)?\s*(?:tk|taka|bdt)?\b/ig,' ').replace(/[–—-]+/g,' ');q=normalizeWebsiteVoice(q).replace(/^and\s+/i,'').replace(/\s+and$/i,'');const f=text.match(/(?:api\s+)?for\s+(.+?)(?=\s+(?:range|price|from|under|below|above|over|between|up to)\b|\s+\d[\d,]*\s*(?:-|to)\s*\d|$)/i);if(f&&f[1].length<80)q=f[1].replace(/\b(?:on|from)\s+\w+\s+(?:website|site)\b/ig,'').trim();return q||'products';}
function websiteTravelIntent(text){const t=String(text||'').trim();if(!/\b(flight|flights|airfare|airline|airport|trip\.com)\b/i.test(t))return null;let origin='',destination='',departDate='';const route=t.match(/(?:from\s+)?([A-Za-z][A-Za-z .'-]{1,40}?)\s+(?:to|→)\s+([A-Za-z][A-Za-z .'-]{1,50}?)(?=\s+(?:on|for|depart|departure|return|check|search|show|find|all|available|flight|flights)\b|[,.]|$)/i);if(route){origin=route[1].replace(/^(?:go|fly|flight|flights|select|search)\s+/i,'').trim();destination=route[2].trim();}const iso=t.match(/\b(20\d{2}-\d{2}-\d{2})\b/);if(iso)departDate=iso[1];return {origin,destination,departDate};}
function parseWebsiteVoice(text){const transcript=normalizeWebsiteVoice(text);if(!transcript)throw new Error('Record or type a voice command first.');let site=websiteVoiceDetectSite(transcript);const travel=websiteTravelIntent(transcript);const askedAny=/\b(any website|any site|best website|best site|choose (?:a|the) website|choose (?:a|the) site|from anywhere)\b/i.test(transcript);if(travel&&(!site||askedAny))site={...WEBSITE_VOICE_SITES.trip,key:'trip',domain:'www.trip.com',autoSelected:true};if(!site||askedAny)site=websiteVoiceBestSite(transcript);const prices=websiteVoicePrices(transcript),query=travel?([travel.origin,travel.destination].filter(Boolean).join(' to ')||'flights'):websiteVoiceQuery(transcript,site),targetUrl=travel?site.base:site.search.replace('{query}',encodeURIComponent(query));const range=prices.maxPrice!=null?`৳${Number(prices.minPrice||0).toLocaleString()}–৳${Number(prices.maxPrice).toLocaleString()}`:prices.minPrice!=null?`above ৳${Number(prices.minPrice).toLocaleString()}`:'';return {id:`webvoice-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,name:travel?`${site.label} ${query} Flight Search API`:`${site.label} ${query}${range?' '+range:''} API`,description:travel?`Autonomous browser API that opens ${site.label}, analyzes the page, enters the requested flight route, searches, verifies the result page, and extracts visible live flight results.`:`Voice-created website API that searches ${site.label} for ${query}${range?` in the ${range} price range`:''} and captures live product data.`,transcript,siteKey:site.key,siteLabel:site.label,domain:site.domain||new URL(site.base).hostname,baseUrl:site.base,autoSelectedSite:!!site.autoSelected,query,minPrice:prices.minPrice,maxPrice:prices.maxPrice,targetUrl,type:'voice',createdAt:new Date().toISOString(),mode:travel?'agent':'catalog',agentType:travel?'travel':null,travel};}
function renderWebsiteVoice(role){
  const wrap=document.getElementById('voice-area-'+role); if(!wrap)return; const v=websiteVoiceState[role],d=v.draft;
  wrap.innerHTML=`<div class="card website-voice-card"><div class="website-voice-head"><div><h2 class="section-title">Create API by Voice</h2><p class="section-sub">Same workflow as the extension: keep recording until you press Stop, review the transcript, create the API, open the real website, test, save, and export JSON.</p></div><span class="website-voice-status ${v.listening?'live':''}">${v.listening?'Listening':'Ready'}</span></div><button class="website-voice-mic ${v.listening?'active':''}" onclick="${v.listening?`stopWebsiteVoice('${role}')`:`startWebsiteVoice('${role}')`}">🎙</button><div class="website-voice-wave ${v.listening?'active':''}"><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div><div class="website-voice-actions">${v.listening?`<button class="btn btn-clay" onclick="stopWebsiteVoice('${role}')">■ Stop Recording</button>`:`<button class="btn btn-primary" onclick="startWebsiteVoice('${role}')">🎙 Start Recording</button>`}${v.transcript?`<button class="btn btn-ghost" onclick="clearWebsiteVoice('${role}')">Clear</button>`:''}</div><div class="field"><label>Live transcript — editable before creating the API</label><textarea id="website-voice-transcript-${role}" rows="5" placeholder="Example: Choose the best website and create an API for shoes from 0 to 3000 taka.">${escapeHtml(v.transcript||'')}</textarea></div>${v.lastError?`<div class="website-voice-error">${escapeHtml(v.lastError)}</div>`:''}<button class="btn btn-primary" onclick="analyzeWebsiteVoice('${role}')" ${v.listening?'disabled':''}>Analyze & Create API</button></div>${d?websiteVoiceDraftHtml(role,d):''}`;
}
function websiteVoiceDraftHtml(role,d){const ran=!!d.lastOutcome;const isAgent=d.agentType==='travel';return `<div class="card website-voice-draft"><div class="flex-between"><div><span class="badge">VOICE DRAFT</span><h3 style="margin:7px 0 4px">${escapeHtml(d.name)}</h3></div><span class="status-chip ${ran?'status-approved':''}">${ran?'Tested':'Ready to test'}</span></div><p class="muted">${escapeHtml(d.description)}</p><div class="website-intent-grid"><div><span>Website</span><b>${escapeHtml(d.siteLabel)}${d.autoSelectedSite?' · auto-selected':''}</b></div><div><span>${isAgent?'Route':'Search'}</span><b>${escapeHtml(d.query)}</b></div><div><span>${isAgent?'Mode':'Price range'}</span><b>${isAgent?'Autonomous page analysis':(d.minPrice!=null||d.maxPrice!=null?`৳ ${Number(d.minPrice||0).toLocaleString()} – ${d.maxPrice!=null?'৳ '+Number(d.maxPrice).toLocaleString():'No max'}`:'Not specified')}</b></div></div><div class="endpoint-box mono"><span>${escapeHtml(d.targetUrl)}</span></div><div class="row" style="gap:8px;flex-wrap:wrap"><button class="btn btn-primary" onclick="runWebsiteVoice('${role}')">▶ Run API</button><button class="btn btn-soft" onclick="saveWebsiteVoice('${role}')">💾 Save API</button><button class="btn btn-soft" onclick="downloadWebsiteVoice('${role}')">⬇ Download JSON</button></div>${ran?`<div class="website-voice-summary"><b>${Number(d.lastOutcome.resultCount||0)}</b> live result${Number(d.lastOutcome.resultCount||0)===1?'':'s'} captured from the real website.</div><pre class="website-json-preview">${escapeHtml(JSON.stringify(d.lastOutcome,null,2))}</pre>`:`<p class="hint">Run opens the real selected website. ${isAgent?'The autonomous agent analyzes the page, clicks the required section, fills your route, searches, verifies the final page, and extracts visible results.':'The catalog runner searches and filters the requested products.'} Save API and Download JSON are available immediately.</p>`}</div>`;}
async function startWebsiteVoice(role){const v=websiteVoiceState[role],SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){v.lastError='Speech recognition is not available in this browser. You can still type the command.';renderWebsiteVoice(role);return;}try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});stream.getTracks().forEach(t=>t.stop());}catch(e){v.lastError='Microphone permission is required.';renderWebsiteVoice(role);return;}if(websiteVoiceRecognition)try{websiteVoiceRecognition.abort();}catch(e){}websiteVoiceFinal=v.transcript?v.transcript.trim()+' ':'';v.listening=true;v.manualStop=false;v.lastError='';v.draft=null;const rec=new SR();websiteVoiceRecognition=rec;rec.continuous=true;rec.interimResults=true;rec.lang='en-US';rec.onresult=e=>{let interim='';for(let i=e.resultIndex;i<e.results.length;i++){const piece=e.results[i][0]?.transcript||'';if(e.results[i].isFinal)websiteVoiceFinal+=piece+' ';else interim+=piece;}v.transcript=normalizeWebsiteVoice(websiteVoiceFinal+interim);const area=document.getElementById(`website-voice-transcript-${role}`);if(area&&document.activeElement!==area)area.value=v.transcript;};rec.onerror=e=>{if(e.error==='aborted'&&v.manualStop)return;v.lastError='Voice recognition: '+(e.error||'unknown error');};rec.onend=()=>{if(v.listening&&!v.manualStop){try{rec.start();}catch(e){}}else{v.listening=false;websiteVoiceRecognition=null;renderWebsiteVoice(role);}};try{rec.start();}catch(e){v.listening=false;v.lastError=e.message||String(e);}renderWebsiteVoice(role);}
function stopWebsiteVoice(role){const v=websiteVoiceState[role];v.manualStop=true;v.listening=false;const area=document.getElementById(`website-voice-transcript-${role}`);if(area)v.transcript=normalizeWebsiteVoice(area.value);if(websiteVoiceRecognition)try{websiteVoiceRecognition.stop();}catch(e){}websiteVoiceRecognition=null;renderWebsiteVoice(role);}
function clearWebsiteVoice(role){if(websiteVoiceState[role].listening)stopWebsiteVoice(role);websiteVoiceState[role]={listening:false,manualStop:false,transcript:'',draft:null,lastError:''};renderWebsiteVoice(role);}
function analyzeWebsiteVoice(role){const v=websiteVoiceState[role],area=document.getElementById(`website-voice-transcript-${role}`);if(area)v.transcript=normalizeWebsiteVoice(area.value);try{v.draft=parseWebsiteVoice(v.transcript);v.lastError='';showToast(v.draft.autoSelectedSite?`Best matching website selected: ${v.draft.siteLabel}`:'Voice API draft created');}catch(e){v.lastError=e.message||String(e);}renderWebsiteVoice(role);}
function waitForWebsiteVoiceResult(timeout=30000){return new Promise((resolve,reject)=>{let done=false;const handler=e=>{if(done)return;done=true;clearTimeout(timer);document.removeEventListener('APIGARDEN_WEB_RUN_VOICE_RESULT',handler);let data={};try{data=JSON.parse(document.documentElement.getAttribute('data-apigarden-web-voice-result')||'{}');}catch(err){data=e.detail||{};}resolve(data);};const timer=setTimeout(()=>{if(done)return;done=true;document.removeEventListener('APIGARDEN_WEB_RUN_VOICE_RESULT',handler);reject(new Error('The extension bridge did not respond quickly enough. Reload the APIGarden extension and try again.'));},timeout);document.addEventListener('APIGARDEN_WEB_RUN_VOICE_RESULT',handler);});}
async function runWebsiteVoice(role){const v=websiteVoiceState[role],d=v.draft;if(!d)return;v.lastError='';renderWebsiteVoice(role);try{const resultPromise=waitForWebsiteVoiceResult();document.documentElement.setAttribute('data-apigarden-web-voice-request',JSON.stringify({intent:d,runId:d.id}));document.dispatchEvent(new CustomEvent('APIGARDEN_WEB_RUN_VOICE'));const res=await resultPromise;if(!res?.ok)throw new Error(res?.error||'Could not run the real website automation.');const ex=res.extraction||{};d.lastOutcome={apiName:d.name,description:d.description,query:d.query,status:ex.agent?.status==='needs_user_action'?'needs_user_action':'success',sourceUrl:ex.sourceUrl||d.targetUrl,pageTitle:ex.pageTitle||'',resultCount:ex.resultCount||0,results:ex.results||[],filters:d.agentType==='travel'?null:{minPrice:d.minPrice??null,maxPrice:d.maxPrice??null,currency:'BDT'},voice:{transcript:d.transcript,website:d.siteLabel,autoSelectedWebsite:!!d.autoSelectedSite},travel:d.travel||null,agent:ex.agent||null,generatedAt:new Date().toISOString(),note:ex.resultCount?(d.agentType==='travel'?'Autonomous browser agent reached a live result page and extracted visible flight results.':'Live visible product data extracted after the real website search and price filter were applied.'):(d.agentType==='travel'?'The autonomous browser agent attempted the workflow but could not verify visible final flight results.':'The real website opened, but no matching visible product cards were detected.')};showToast(`${d.lastOutcome.resultCount} matching live products captured`);}catch(e){v.lastError=e.message||String(e);window.open(d.targetUrl,'_blank');}renderWebsiteVoice(role);}
function websiteVoiceExportPayload(d){return d.lastOutcome||{apiName:d.name,description:d.description,query:d.query,status:'draft',sourceUrl:d.targetUrl,pageTitle:'',resultCount:0,results:[],filters:{minPrice:d.minPrice??null,maxPrice:d.maxPrice??null,currency:'BDT'},voice:{transcript:d.transcript,website:d.siteLabel,autoSelectedWebsite:!!d.autoSelectedSite},createdAt:d.createdAt||new Date().toISOString(),generatedAt:new Date().toISOString(),note:'Draft Voice API. Run API to replace this draft with live visible product data from the selected website.'};}
function saveWebsiteVoice(role){const d=websiteVoiceState[role].draft;if(!d)return;const existing=state[role].apis.findIndex(a=>a.type==='voice'&&(a.voiceIntent?.id===d.id||a.endpoint===d.targetUrl&&a.name===d.name));const api={name:d.name,description:d.description,endpoint:d.targetUrl,runs:d.lastOutcome?1:0,owner:state[role].email,ownershipStatus:'Owned',type:'voice',voiceIntent:{...d,lastOutcome:undefined},lastOutcome:d.lastOutcome||null};if(existing>=0)state[role].apis[existing]={...state[role].apis[existing],...api,runs:Math.max(state[role].apis[existing].runs||0,api.runs)};else state[role].apis.unshift(api);userStore[`${role}:${state[role].email.toLowerCase()}`]=state[role];saveAppData();renderMyApis(role);showToast(d.lastOutcome?'Voice API saved with live result':'Voice API draft saved to My APIs');}
function downloadWebsiteVoice(role){const d=websiteVoiceState[role].draft;if(!d)return;const payload=websiteVoiceExportPayload(d),blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=(d.name||'voice-api').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')+(d.lastOutcome?'-outcome.json':'-draft.json');document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}
    ...
    ${apis.map((a,i)=>`
      <div class="card api-card">
        ...
        <div class="body">
          ...
          ${a.type==='voice'?`<button class="btn btn-primary small-btn" onclick="runSavedWebsiteVoice('${role}', ${i})">Run on website</button><button class="btn btn-soft small-btn" onclick="downloadSavedWebsiteVoice('${role}', ${i})">Download JSON</button>`:`<button class="btn btn-primary small-btn" onclick="runMyApi('${role}', ${i})">Run</button><button class="btn btn-soft small-btn" onclick="runMyApiWithPage('${role}', ${i})">Analyze current page</button>`}
...
async function runSavedWebsiteVoice(role,i){const api=state[role].apis[i];if(!api||api.type!=='voice')return;const d={...(api.voiceIntent||{}),name:api.name,description:api.description,targetUrl:api.endpoint||api.voiceIntent?.targetUrl};websiteVoiceState[role].draft=d;await runWebsiteVoice(role);if(websiteVoiceState[role].draft?.lastOutcome){api.lastOutcome=websiteVoiceState[role].draft.lastOutcome;api.runs=(api.runs||0)+1;saveAppData();renderMyApis(role);}}
function downloadSavedWebsiteVoice(role,i){const api=state[role].apis[i];if(!api)return;const d={...(api.voiceIntent||{}),name:api.name,description:api.description,targetUrl:api.endpoint||api.voiceIntent?.targetUrl,lastOutcome:api.lastOutcome||null};const payload=websiteVoiceExportPayload(d),blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=(api.name||'voice-api').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')+(api.lastOutcome?'-outcome.json':'-draft.json');document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}
