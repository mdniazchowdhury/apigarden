...
function parseWebsiteVoice(text){const transcript=normalizeWebsiteVoice(text);if(!transcript)throw new Error('Record or type a voice command first.');let site=websiteVoiceDetectSite(transcript);const askedAny=/\b(any website|any site|best website|best site|choose (?:a|the) website|choose (?:a|the) site|from anywhere)\b/i.test(transcript);if(!site||askedAny)site=websiteVoiceBestSite(transcript);const prices=websiteVoicePrices(transcript),query=websiteVoiceQuery(transcript,site),targetUrl=site.search.replace('{query}',encodeURIComponent(query));const range=prices.maxPrice!=null?`৳${Number(prices.minPrice||0).toLocaleString()}–৳${Number(prices.maxPrice).toLocaleString()}`:prices.minPrice!=null?`above ৳${Number(prices.minPrice).toLocaleString()}`:'';return {id:`webvoice-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,name:`${site.label} ${query}${range?' '+range:''} API`,description:`Voice-created website API that searches ${site.label} for ${query}${range?` in the ${range} price range`:''} and captures live product data.`,transcript,siteKey:site.key,siteLabel:site.label,domain:site.domain,baseUrl:site.base,autoSelectedSite:!!site.autoSelected,query,minPrice:prices.minPrice,maxPrice:prices.maxPrice,targetUrl,type:'voice',createdAt:new Date().toISOString()};}
...
function websiteVoiceDraftHtml(role,d){const ran=!!d.lastOutcome;return `<div class="card website-voice-draft"><div class="flex-between"><div><span class="badge">VOICE DRAFT</span><h3 style="margin:7px 0 4px">${escapeHtml(d.name)}</h3></div><span class="status-chip ${ran?'status-approved':''}">${ran?'Tested':'Ready to test'}</span></div><p class="muted">${escapeHtml(d.description)}</p><div class="website-intent-grid"><div><span>Website</span><b>${escapeHtml(d.siteLabel)}${d.autoSelectedSite?' · auto-selected':''}</b></div><div><span>Search</span><b>${escapeHtml(d.query)}</b></div><div><span>Price range</span><b>${d.minPrice!=null||d.maxPrice!=null?`৳ ${Number(d.minPrice||0).toLocaleString()} – ${d.maxPrice!=null?'৳ '+Number(d.maxPrice).toLocaleString():'No max'}`:'Not specified'}</b></div></div><div class="endpoint-box mono"><span>${escapeHtml(d.targetUrl)}</span></div><div class="row" style="gap:8px;flex-wrap:wrap"><button class="btn btn-primary" onclick="runWebsiteVoice('${role}')">▶ Run API</button><button class="btn btn-soft" onclick="saveWebsiteVoice('${role}')">💾 Save API</button><button class="btn btn-soft" onclick="downloadWebsiteVoice('${role}')">⬇ Download JSON</button></div>${ran?`<div class="website-voice-summary"><b>${Number(d.lastOutcome.resultCount||0)}</b> matching live products captured from the real website.</div><pre class="website-json-preview">${escapeHtml(JSON.stringify(d.lastOutcome,null,2))}</pre>`:`<p class="hint">Run opens the real selected website. Save API is available immediately; Download JSON exports the draft now and automatically exports the live result after a successful run.</p>`}</div>`;}
