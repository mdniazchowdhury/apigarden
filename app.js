...
function websiteVoiceDraftHtml(role,d){const ran=!!d.lastOutcome;return `<div class="card website-voice-draft"><div class="flex-between"><div><span class="badge">VOICE DRAFT</span><h3 style="margin:7px 0 4px">${escapeHtml(d.name)}</h3></div><span class="status-chip ${ran?'status-approved':''}">${ran?'Tested':'Ready to test'}</span></div><p class="muted">${escapeHtml(d.description)}</p><div class="website-intent-grid"><div><span>Website</span><b>${escapeHtml(d.siteLabel)}${d.autoSelectedSite?' · auto-selected':''}</b></div><div><span>Search</span><b>${escapeHtml(d.query)}</b></div><div><span>Price range</span><b>${d.minPrice!=null||d.maxPrice!=null?`৳ ${Number(d.minPrice||0).toLocaleString()} – ${d.maxPrice!=null?'৳ '+Number(d.maxPrice).toLocaleString():'No max'}`:'Not specified'}</b></div></div><div class="endpoint-box mono"><span>${escapeHtml(d.targetUrl)}</span></div><div class="row" style="gap:8px;flex-wrap:wrap"><button class="btn btn-primary" onclick="runWebsiteVoice('${role}')">▶ Run API</button>${ran?`<button class="btn btn-soft" onclick="saveWebsiteVoice('${role}')">Save API</button><button class="btn btn-soft" onclick="downloadWebsiteVoice('${role}')">Download JSON</button>`:''}</div>${ran?`<div class="website-voice-summary"><b>${Number(d.lastOutcome.resultCount||0)}</b> matching live products captured from the real website.</div><pre class="website-json-preview">${escapeHtml(JSON.stringify(d.lastOutcome,null,2))}</pre>`:`<p class="hint">Run opens the real selected website. When the APIGarden extension is installed, the website uses the same extension automation engine to filter and capture live product data.</p>`}</div>`;}
...
function saveWebsiteVoice(role){const d=websiteVoiceState[role].draft;if(!d?.lastOutcome){showToast('Run the API first');return;}state[role].apis.unshift({name:d.name,description:d.description,endpoint:d.targetUrl,runs:1,owner:state[role].email,ownershipStatus:'Owned',type:'voice',voiceIntent:{...d,lastOutcome:undefined},lastOutcome:d.lastOutcome});userStore[`${role}:${state[role].email.toLowerCase()}`]=state[role];saveAppData();renderMyApis(role);showToast('Voice API saved to My APIs');}
function downloadWebsiteVoice(role){const d=websiteVoiceState[role].draft;if(!d?.lastOutcome)return;const blob=new Blob([JSON.stringify(d.lastOutcome,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=(d.name||'voice-api').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')+'-outcome.json';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}
...
function downloadSavedWebsiteVoice(role,i){const api=state[role].apis[i];if(!api?.lastOutcome){showToast('Run this voice API first');return;}const blob=new Blob([JSON.stringify(api.lastOutcome,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=(api.name||'voice-api').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')+'-outcome.json';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}
